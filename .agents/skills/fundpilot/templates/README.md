...
### 3. daily-position-analysis-template.html

周期性 Agent 日度持仓分析模板，用于生成 `../analysis-history/` 下的历史报告，并由 `../index.html` 统一链接。

**特点**：
- 固定分析结构，便于历史报告横向对比
- 覆盖持仓、行情、策略信号、资讯、操作建议和风险提示
- 明确记录数据来源与缺口，避免行情或资讯不可用时误导用户

**适用场景**：天级调度任务完成持仓分析后，沉淀一份可回看的 HTML 报告。
...
- 周期性日度持仓分析必须使用 `daily-position-analysis-template.html`，并同步更新 `../index.html` 的历史链接