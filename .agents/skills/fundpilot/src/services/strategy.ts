...
import type { Signal, Quote, StrategyRule, Plan, Position } from '../types';
import { getStrategy, listPlans, listPositions, listFunds } from '../db';
import { getQuote, getAllQuotes } from './quote';

/**
 * 计算今日信号
 */
export async function calculateTodaySignal(code: string, strategyName: string = 'default'): Promise<Signal | null> {
  const fund = (await listFunds()).find(f => f.code === code);
  if (!fund) {
    return null;
  }
 ...
 * 计算所有基金的今日信号
 */
export async function calculateAllSignals(strategyName: string = 'default'): Promise<Signal[]> {
  const strategy = await getStrategy(strategyName);
  if (!strategy) {
    return [];
  }
...
function calculateSignal(
  ...
  // 默认信号
  let action: 'buy' | 'hold' | 'pause' = 'hold';
  let amount = 0;
  let reason = '';
 ...
 * 检查是否应该在今天执行定投
 */
export function shouldExecuteToday(plan: Plan): boolean {
