...
### 行情接口失败时的 Agent Fallback（检索补数，不写入 CLI）

CLI **不**内置第二套行情源。当 `position upsert` 因拉不到天天基金数据而报 `MISSING_COST`、或 `signal` / `today-all` 报行情拉取失败时，**由 Agent 自行**用 WebSearch、网页抓取等工具补数后再调用 CLI：

1. **检索**：`{基金代码} 最新净值`、`{基金简称} 单位净值`（示例：`019670 最新净值`）。
2. **核对**：以基金销售机构或财经站点披露的**净值日期 + 单位净值**为准；页面日期可能略早于当日，取**已披露的最新一行**即可。
3. **回写**：用显式成本写入持仓，例如  
   `position upsert --code=019670 --shares=84.41 --cost=1.2913 --json`  
   （`--cost` 填上一步确认的单位净值；份额仍以用户/本地数据为准。）
4. **告知用户**：说明净值来源网站与日期；第三方与代销 APP 可能存在微小差异，若用户以平台为准可自行再执行一次 `upsert` 修正 `--cost`。

**实例（019670）**：证券之星基金净值页可列出逐日净值；曾出现天天基金 JSONP 拉取失败而第三方页面仍可读的情况——此类场景即适合走上述 Fallback，**不得**编造净值。
...
所有数据存储在本 skill 目录下的 `.fundpilot/fundpilot.db`（与 `package.json` 同级，SQLite 数据库）。**该文件纳入 Git 版本管理**，便于多环境同步；若含真实持仓且仓库为公开，请注意隐私与合规。

## 注意事项

1. **行情数据**: 仅使用天天基金接口拉取；失败时相关命令会报错，不会使用模拟数据。**Agent** 可按上文「行情接口失败时的 Agent Fallback」自行检索补数后再传 `--cost`。
